...
// BusinessDayCalculator SDK pipeline test
//
...
{
    private static function utility(): BusinessDayCalculatorUtility
    {
        return new BusinessDayCalculatorUtility();
    }
    ...
    private static function ctx(array $over = []): BusinessDayCalculatorContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BusinessDayCalculatorContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BusinessDayCalculatorOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BusinessDayCalculatorError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BusinessDayCalculatorOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BusinessDayCalculatorMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BusinessDayCalculatorOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BusinessDayCalculatorMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BusinessDayCalculatorOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BusinessDayCalculatorMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BusinessDayCalculatorSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BusinessDayCalculatorMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BusinessDayCalculatorResponse([]),
            'result' => new BusinessDayCalculatorResult([]),
        ]);
        [, $err] = BusinessDayCalculatorMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BusinessDayCalculatorSpec([]),
            'response' => null,
            'result' => new BusinessDayCalculatorResult([]),
        ]);
        [, $err] = BusinessDayCalculatorMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BusinessDayCalculatorSpec(['step' => 's']),
            'response' => new BusinessDayCalculatorResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BusinessDayCalculatorResult([]),
        ]);
        [, $err] = BusinessDayCalculatorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BusinessDayCalculatorSpec(['step' => 's']),
            'response' => new BusinessDayCalculatorResponse(self::resp(200, ['v' => 1])),
            'result' => new BusinessDayCalculatorResult([]),
        ]);
        [, $err] = BusinessDayCalculatorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BusinessDayCalculatorSpec(['step' => 's']),
            'response' => new BusinessDayCalculatorResponse(self::resp(200, ['v' => 2])),
            'result' => new BusinessDayCalculatorResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BusinessDayCalculatorMakeResponse::call($ctx);
    ...
    {
        $preset = new BusinessDayCalculatorResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BusinessDayCalculatorSpec([]),
            'response' => new BusinessDayCalculatorResponse([]),
            'result' => new BusinessDayCalculatorResult([]),
        ...
        [$got, $err] = BusinessDayCalculatorMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BusinessDayCalculatorResult([])]);
        [, $err] = BusinessDayCalculatorMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BusinessDayCalculatorSpec([]), 'result' => null]);
        [, $err] = BusinessDayCalculatorMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BusinessDayCalculatorSpec(['step' => 's']),
            'result' => new BusinessDayCalculatorResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BusinessDayCalculatorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BusinessDayCalculatorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BusinessDayCalculatorSpec(['step' => 's']),
            'result' => new BusinessDayCalculatorResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BusinessDayCalculatorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BusinessDayCalculatorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BusinessDayCalculatorResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BusinessDayCalculatorSpec([]),
            'result' => new BusinessDayCalculatorResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BusinessDayCalculatorMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BusinessDayCalculatorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BusinessDayCalculatorError('boom', 'boom');
        $utility->fetcher = function (BusinessDayCalculatorContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BusinessDayCalculatorSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BusinessDayCalculatorMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BusinessDayCalculatorContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BusinessDayCalculatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BusinessDayCalculatorResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BusinessDayCalculatorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BusinessDayCalculatorContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BusinessDayCalculatorSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BusinessDayCalculatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BusinessDayCalculatorResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BusinessDayCalculatorSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BusinessDayCalculatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BusinessDayCalculatorMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BusinessDayCalculatorMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BusinessDayCalculatorResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BusinessDayCalculatorDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BusinessDayCalculatorResult(['ok' => false])]);
        $this->expectException(BusinessDayCalculatorError::class);
        BusinessDayCalculatorDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BusinessDayCalculatorResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BusinessDayCalculatorMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BusinessDayCalculatorMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BusinessDayCalculatorBaseFeature();
        $b = new BusinessDayCalculatorBaseFeature();
        BusinessDayCalculatorFeatureAdd::call($ctx, $a);
        BusinessDayCalculatorFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BusinessDayCalculatorBaseFeature
    {
        $f = new BusinessDayCalculatorBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BusinessDayCalculatorFeatureAdd::call($ctx, self::named_feature('a'));
        BusinessDayCalculatorFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        BusinessDayCalculatorFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        BusinessDayCalculatorFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        BusinessDayCalculatorFeatureAdd::call($ctx, $replace);
        ...
        BusinessDayCalculatorFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BusinessDayCalculatorContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BusinessDayCalculatorSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BusinessDayCalculatorPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        BusinessDayCalculatorPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BusinessDayCalculatorResponse(['headers' => null]),
            'result' => new BusinessDayCalculatorResult([]),
        ]);
        BusinessDayCalculatorResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        BusinessDayCalculatorResultBody::call($ctx);

...
class DomainEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = BusinessDayCalculatorSDK::test($seed, null);
        $seen = iterator_to_array($base->Domain(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = BusinessDayCalculatorConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = BusinessDayCalculatorSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BUSINESS_DAY_CALCULATOR_TEST_DOMAIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function domain_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BUSINESS_DAY_CALCULATOR_TEST_DOMAIN_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BUSINESS_DAY_CALCULATOR_TEST_DOMAIN_ENTID"]);
    ...
    if ($env["BUSINESS_DAY_CALCULATOR_TEST_LIVE"] === "TRUE") {
        ...
        $client = new BusinessDayCalculatorSDK(Helpers::to_map($merged_opts));
    }

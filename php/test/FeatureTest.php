...
// BusinessDayCalculator SDK feature test
//
...
class FtCtrl extends BusinessDayCalculatorControl
{
...
class FtClient
{
    ...
    private BusinessDayCalculatorUtility $utility;

    public function __construct(BusinessDayCalculatorUtility $utility, array $options)
    {
    ...
    public function get_utility(): BusinessDayCalculatorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BusinessDayCalculatorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BusinessDayCalculatorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BusinessDayCalculatorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BusinessDayCalculatorConfig::make_feature($name);
    ...
    private static function buildUrl(BusinessDayCalculatorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BusinessDayCalculatorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BusinessDayCalculatorResponse($fetched);
        }
    ...
    private function fail(BusinessDayCalculatorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BusinessDayCalculatorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

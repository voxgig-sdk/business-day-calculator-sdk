...
// BusinessDayCalculator SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = BusinessDayCalculatorOmni::makeStructRunner(
    ...
    {
        return BusinessDayCalculatorOmni::objmodel(BusinessDayCalculatorOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return BusinessDayCalculatorOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return BusinessDayCalculatorOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(BusinessDayCalculatorOmni::structfix($now, Struct::undef()))
                        !== json_encode(BusinessDayCalculatorOmni::structfix($val, Struct::undef()))) {

...
class SslEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BUSINESSDAYCALCULATOR_TEST_SSL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function ssl_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BUSINESSDAYCALCULATOR_TEST_SSL_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "BUSINESSDAYCALCULATOR_TEST_SSL_ENTID" => $idmap,
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BUSINESSDAYCALCULATOR_TEST_SSL_ENTID"]);

# BusinessDayCalculator PHP SDK



The PHP SDK for the BusinessDayCalculator API — an entity-oriented client using PHP conventions.
...
```php
<?php
require_once 'businessdaycalculator_sdk.php';

$client = new BusinessDayCalculatorSDK([
    "apikey" => getenv("BUSINESS_DAY_CALCULATOR_APIKEY"),
]);
```
...
```php
$client = BusinessDayCalculatorSDK::test();
...
```php
require_once 'businessdaycalculator_sdk.php';
$client = new BusinessDayCalculatorSDK($options);
```
...
```php
$client = BusinessDayCalculatorSDK::test($testopts, $sdkopts);
```
...
```
php/
├── businessdaycalculator_sdk.php          -- Main SDK class
...
The main class (`businessdaycalculator_sdk.php`) exports the SDK class

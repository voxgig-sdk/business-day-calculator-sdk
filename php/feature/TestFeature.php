...
// BusinessDayCalculator SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BusinessDayCalculatorTestFeature extends BusinessDayCalculatorBaseFeature
{
    ...
    public function init(BusinessDayCalculatorContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BusinessDayCalculatorContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BusinessDayCalculatorParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BusinessDayCalculatorContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BusinessDayCalculatorContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BusinessDayCalculatorParam::call($ctx, $k);

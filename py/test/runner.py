# BusinessDayCalculator SDK test runner
...
class BusinessDayCalculatorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BusinessDayCalculatorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BusinessDayCalculatorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BusinessDayCalculatorTestRunner.getenv("BUSINESS_DAY_CALCULATOR_TEST_LIVE")
        override = BusinessDayCalculatorTestRunner.getenv("BUSINESS_DAY_CALCULATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BusinessDayCalculatorTestRunner.getenv(key)
        ...
        explain = BusinessDayCalculatorTestRunner.getenv("BUSINESS_DAY_CALCULATOR_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BUSINESS_DAY_CALCULATOR_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BusinessDayCalculatorTestRunner._test_control is not None:
            return BusinessDayCalculatorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BusinessDayCalculatorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BusinessDayCalculatorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BusinessDayCalculatorTestRunner.load_test_control()
...
def load_env_local():
    BusinessDayCalculatorTestRunner.load_env_local()


def env_override(m):
    return BusinessDayCalculatorTestRunner.env_override(m)


def entity_data(v):
    return BusinessDayCalculatorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BusinessDayCalculatorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BusinessDayCalculatorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BusinessDayCalculatorTestRunner.load_test_control()


def live_client_options():
    return BusinessDayCalculatorTestRunner.live_client_options()


def live_delay_ms():
    return BusinessDayCalculatorTestRunner.live_delay_ms()

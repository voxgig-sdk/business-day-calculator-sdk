    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BUSINESSDAYCALCULATOR_TEST_EMAIL_VALIDATE_ENTID JSON to run live")
        client = setup["client"]
...
def _email_validate_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "BUSINESSDAYCALCULATOR_TEST_EMAIL_VALIDATE_ENTID")
    _idmap_overridden = _entid_env_raw is not None and _entid_env_raw.strip().startswith("{")

    env = runner.env_override({
        "BUSINESSDAYCALCULATOR_TEST_EMAIL_VALIDATE_ENTID": idmap,
    ...
    idmap_resolved = helpers.to_map(
        env.get("BUSINESSDAYCALCULATOR_TEST_EMAIL_VALIDATE_ENTID"))

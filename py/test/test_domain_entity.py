...
class TestDomainEntity:
    ...
    def test_should_stream(self):
        ...
        base = BusinessDayCalculatorSDK.test(seed, None)
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = BusinessDayCalculatorSDK.test(
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BUSINESS_DAY_CALCULATOR_TEST_DOMAIN_ENTID JSON to run live")
        client = setup["client"]
...
def _domain_basic_setup(extra):
    ...
    client = BusinessDayCalculatorSDK.test(options, extra)
    ...
    idmap_resolved = helpers.to_map(
        env.get("BUSINESS_DAY_CALCULATOR_TEST_DOMAIN_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("BUSINESS_DAY_CALCULATOR_TEST_LIVE") == "TRUE":
        ...
        client = BusinessDayCalculatorSDK(helpers.to_map(merged_opts))

    _live = env.get("BUSINESS_DAY_CALCULATOR_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BUSINESS_DAY_CALCULATOR_TEST_EXPLAIN") == "TRUE",
        "live": _live,

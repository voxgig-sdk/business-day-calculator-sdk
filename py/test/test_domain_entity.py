    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BUSINESSDAYCALCULATOR_TEST_DOMAIN_ENTID JSON to run live")
        client = setup["client"]
...
def _domain_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "BUSINESSDAYCALCULATOR_TEST_DOMAIN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BUSINESSDAYCALCULATOR_TEST_DOMAIN_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("BUSINESSDAYCALCULATOR_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("BUSINESSDAYCALCULATOR_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BUSINESSDAYCALCULATOR_TEST_EXPLAIN") == "TRUE",
        "live": _live,

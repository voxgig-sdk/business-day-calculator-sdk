...
from businessdaycalculator_sdk.core.helpers import get_ctx_prop, to_map
...
class BusinessDayCalculatorContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = BusinessDayCalculatorControl()
        ...
        if isinstance(sp, BusinessDayCalculatorSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, BusinessDayCalculatorResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, BusinessDayCalculatorResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return BusinessDayCalculatorOperation({})
        ...
        op = BusinessDayCalculatorOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return BusinessDayCalculatorError(code, msg, self)

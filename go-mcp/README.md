# business-day-calculator-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BusinessDayCalculator SDK as
two agent tools — `business-day-calculator_list` and `business-day-calculator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/business-day-calculator-mcp)
make build

# 2. Provide credentials via the environment
export BUSINESS_DAY_CALCULATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user business-day-calculator \
  -- /absolute/path/to/business-day-calculator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./business-day-calculator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// business-day-calculator_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// business-day-calculator_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/business-day-calculator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user business-day-calculator \
     -- "$PWD"/dist/*/business-day-calculator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `business-day-calculator_list` and `business-day-calculator_load` tools now appear
   in new sessions. Ask the agent to *"list domain using business-day-calculator"*
   and it calls `business-day-calculator_list` with `{"entity":"domain"}`.
...
```sh
export BUSINESS_DAY_CALCULATOR_APIKEY=sk_live_xxx            # API key
export BUSINESS_DAY_CALCULATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `business-day-calculator_list` tool
...
### Call the `business-day-calculator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `business-day-calculator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `business-day-calculator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BUSINESS_DAY_CALCULATOR_APIKEY` | API key sent with every request. |
| `BUSINESS_DAY_CALCULATOR_BASE` | Optional override of the API base URL. |
...
```sh
./business-day-calculator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"business-day-calculator_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```

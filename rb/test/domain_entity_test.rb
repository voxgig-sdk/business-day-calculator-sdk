...
class DomainEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = BusinessDayCalculatorSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BUSINESS_DAY_CALCULATOR_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["BUSINESS_DAY_CALCULATOR_TEST_DOMAIN_ENTID"]
  ...
  if env["BUSINESS_DAY_CALCULATOR_TEST_LIVE"] == "TRUE"
    ...
    client = BusinessDayCalculatorSDK.new(Helpers.to_map(merged_opts))
  end

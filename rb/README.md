# BusinessDayCalculator Ruby SDK



The Ruby SDK for the BusinessDayCalculator API — an entity-oriented client using idiomatic Ruby conventions.
...
client = BusinessDayCalculatorSDK.new({
...
```ruby
require_relative "BusinessDayCalculator_sdk"
client = BusinessDayCalculatorSDK.new(options)
```
...
raise a `BusinessDayCalculatorError` (a `StandardError` subclass), so wrap
...
```
rb/
├── BusinessDayCalculator_sdk.rb       -- Main SDK module
...
The main module (`BusinessDayCalculator_sdk`) exports the SDK class

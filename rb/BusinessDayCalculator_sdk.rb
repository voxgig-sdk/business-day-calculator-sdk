...
require_relative 'BusinessDayCalculator_types'
...
class BusinessDayCalculatorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BusinessDayCalculatorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BusinessDayCalculatorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BusinessDayCalculatorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BusinessDayCalculatorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BusinessDayCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BusinessDayCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BusinessDayCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BusinessDayCalculatorError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue BusinessDayCalculatorError => err
    ...
    if fetched.is_a?(Hash)
      status = BusinessDayCalculatorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BusinessDayCalculatorError.new(
        "graphql_error", "BusinessDayCalculatorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BusinessDayCalculatorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

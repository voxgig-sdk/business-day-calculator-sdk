...
module BusinessDayCalculatorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BusinessDayCalculatorResponse.new({})
    ...
    if fetch_err
      ...
      response = BusinessDayCalculatorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BusinessDayCalculatorResponse.new(fetched)
    else

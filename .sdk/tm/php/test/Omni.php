...
//    only - a typed BusinessDayCalculatorContext there would make every ctx
...
final class BusinessDayCalculatorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BusinessDayCalculatorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BusinessDayCalculatorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BusinessDayCalculatorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BusinessDayCalculatorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BusinessDayCalculatorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BusinessDayCalculatorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BusinessDayCalculatorError(is_string($code) ? $code : '', $msg);
    }

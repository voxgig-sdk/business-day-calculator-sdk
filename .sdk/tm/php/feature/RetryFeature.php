...
// BusinessDayCalculator SDK retry feature
...
class BusinessDayCalculatorRetryFeature extends BusinessDayCalculatorBaseFeature
{
    ...
    public function init(BusinessDayCalculatorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BusinessDayCalculatorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BusinessDayCalculatorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BusinessDayCalculatorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

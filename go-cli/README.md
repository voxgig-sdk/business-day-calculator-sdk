# business-day-calculator-cli

boru-driven command-line client **and** interactive REPL for the BusinessDayCalculator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/business-day-calculator-cli)
make build

# 2. See usage (words, entities, env vars)
./business-day-calculator-cli --help

# 3. Provide credentials once, via the environment
export BUSINESS_DAY_CALCULATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./business-day-calculator-cli load 1 dns_result            # {id:1} shorthand
./business-day-calculator-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
BUSINESS_DAY_CALCULATOR_BASE=https://api.example.com ./business-day-calculator-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./business-day-calculator-cli
business-day-calculator> load 1 dns_result
business-day-calculator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/business-day-calculator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/business-day-calculator-cli load 1 dns_result
   ```
...
```sh
./business-day-calculator-cli load 1 dns_result          # scalar shorthand for {id:1}
./business-day-calculator-cli load '{id:1}' dns_result     # explicit match map
```
...
./business-day-calculator-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `business-day-calculator>`). Each line is
evaluated as its own boru expression:

```text
$ ./business-day-calculator-cli
business-day-calculator> load 1 dns_result
business-day-calculator> /help
business-day-calculator> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/business-day-calculator-cli`. |
...
(`./business-day-calculator-cli <expr>`) and interactively (the REPL), and expressions compose the

...
require("business-day-calculator_types")
...
local BusinessDayCalculatorSDK = {}
BusinessDayCalculatorSDK.__index = BusinessDayCalculatorSDK
...
BusinessDayCalculatorSDK._make_feature = _make_feature


function BusinessDayCalculatorSDK.new(options)
  local self = setmetatable({}, BusinessDayCalculatorSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BusinessDayCalculatorSDK:options_map()
...
function BusinessDayCalculatorSDK:get_utility()
  return Utility.copy(self._utility)
end


function BusinessDayCalculatorSDK:get_root_ctx()
  return self._rootctx
end


function BusinessDayCalculatorSDK:prepare(fetchargs)
...
function BusinessDayCalculatorSDK:direct(fetchargs)
...
function BusinessDayCalculatorSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function BusinessDayCalculatorSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "BusinessDayCalculatorSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function BusinessDayCalculatorSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BusinessDayCalculatorSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BusinessDayCalculatorSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BusinessDayCalculatorSDK:DnsResult(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:Domain(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:EmailValidate(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:Generate(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:Grammar(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:Ipn(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:Redact(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:Ssl(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:Utility(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BusinessDayCalculatorSDK:Whoi(data)
...
function BusinessDayCalculatorSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BusinessDayCalculatorSDK.new(sdkopts)
  sdk.mode = "test"
...
return BusinessDayCalculatorSDK

...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BUSINESS_DAY_CALCULATOR_TEST_LIVE") == "TRUE" ||
		os.Getenv("BUSINESS_DAY_CALCULATOR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BUSINESS_DAY_CALCULATOR_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.BusinessDayCalculatorSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.BusinessDayCalculatorSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.BusinessDayCalculatorError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.BusinessDayCalculatorSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.BusinessDayCalculatorError{Msg: msg, Code: code}
}

...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BUSINESSDAYCALCULATOR_TEST_LIVE") == "TRUE" ||
		os.Getenv("BUSINESSDAYCALCULATOR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BUSINESSDAYCALCULATOR_TEST_EXPLAIN"); explain != "" {

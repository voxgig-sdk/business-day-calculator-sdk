...
import (
	...
	omni "github.com/voxgig-sdk/business-day-calculator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BusinessDayCalculatorError); is {
				out := map[string]any{
					"name":    "BusinessDayCalculatorError",
...
func omniCtx(arg any, client *sdk.BusinessDayCalculatorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BusinessDayCalculatorSDK); is {
			client = live

...
func TestDnsResultEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BUSINESSDAYCALCULATOR_TEST_DNS_RESULT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func dns_resultBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BUSINESSDAYCALCULATOR_TEST_DNS_RESULT_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"BUSINESSDAYCALCULATOR_TEST_DNS_RESULT_ENTID": idmap,
		"BUSINESSDAYCALCULATOR_TEST_LIVE":      "FALSE",
		"BUSINESSDAYCALCULATOR_TEST_EXPLAIN":   "FALSE",
		"BUSINESSDAYCALCULATOR_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["BUSINESSDAYCALCULATOR_TEST_DNS_RESULT_ENTID"])

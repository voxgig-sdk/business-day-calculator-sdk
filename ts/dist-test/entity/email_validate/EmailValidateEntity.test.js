...
(0, node_test_1.describe)('EmailValidateEntity', async () => {
    ...
    // `test.live.delayMs`; only sleeps when BUSINESS_DAY_CALCULATOR_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('BUSINESS_DAY_CALCULATOR_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.BusinessDayCalculatorSDK.test();
        const ent = testsdk.EmailValidate();
        (0, node_assert_1.default)(null != ent);
    });
    (0, node_test_1.test)('basic', async (t) => {
        const live = 'TRUE' === process.env.BUSINESS_DAY_CALCULATOR_TEST_LIVE;
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set BUSINESS_DAY_CALCULATOR_TEST_EMAIL_VALIDATE_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let client = __1.BusinessDayCalculatorSDK.test(options, extra);
    ...
    const idmapEnvVal = process.env['BUSINESS_DAY_CALCULATOR_TEST_EMAIL_VALIDATE_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'BUSINESS_DAY_CALCULATOR_TEST_EMAIL_VALIDATE_ENTID': idmap,
        'BUSINESS_DAY_CALCULATOR_TEST_LIVE': 'FALSE',
        'BUSINESS_DAY_CALCULATOR_TEST_EXPLAIN': 'FALSE',
        'BUSINESS_DAY_CALCULATOR_APIKEY': '',
    });
    idmap = env['BUSINESS_DAY_CALCULATOR_TEST_EMAIL_VALIDATE_ENTID'];
    const live = 'TRUE' === env.BUSINESS_DAY_CALCULATOR_TEST_LIVE;
    if (live) {
        client = new __1.BusinessDayCalculatorSDK(merge([
    ...
    const setup = {
        ...
        struct,
        data: entityData,
        explain: 'TRUE' === env.BUSINESS_DAY_CALCULATOR_TEST_EXPLAIN,
        live,

...
const __1 = require("..");
const SDK_NAME = 'BusinessDayCalculatorSDK';
...
function isRunnable(code) {
    return (/new\s+BusinessDayCalculatorSDK\b/.test(code) ||
        /\bBusinessDayCalculatorSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BusinessDayCalculatorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BusinessDayCalculatorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
